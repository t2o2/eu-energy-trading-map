...
for (const feature of fc.features) {
	const iso2 =
		numericToIso2.get(String(feature.id)) ??
		ISO2_BY_NAME[feature.properties?.name];
	if (!iso2) continue;
...
function densify(line, stepKm = 12) {
	...
	for (let i = 0; i <= n; i++) {
		points.push(
			turf.along(line, (length * i) / n, { units: "kilometers" }).geometry
				.coordinates,
		);
	}
	return points;
...
/**
 * Anchor + bearing for a land border, from the ordered run of shared vertices.
 * The anchor is the point halfway *along* the run rather than the centroid of
 * its bounding box: a curved border (the Norway/Sweden ridge, the Ore
 * Mountains) bends away from its own centroid, which left arrows floating tens
 * of kilometres inside one of the countries. The bearing is the local normal
 * to the border, flipped to whichever side faces B, so the arrow crosses the
 * boundary instead of running along it.
 */
function landAnchor(run, featA, featB) {
	const line = turf.lineString(run);
	const length = turf.length(line, { units: "kilometers" });
	const anchor =
		length > 0
			? turf.along(line, length / 2, { units: "kilometers" }).geometry
					.coordinates
			: run[0];
	...
	return {
		anchor,
		line,
		bearing: crossingBearing(line, anchor, featA, featB),
	};
}

/**
 * Bearing perpendicular to the border at `anchor`, pointing from A's side to
 * B's side, so the arrow crosses the boundary instead of running along it.
 */
function crossingBearing(line, anchor, featA, featB) {
	const length = turf.length(line, { units: "kilometers" });
	const at = turf.nearestPointOnLine(line, turf.point(anchor), {
		units: "kilometers",
	}).properties.location;

	// Tangent over a short span around the anchor; a single vertex pair is too
	// noisy on densified outlines.
	const span = Math.min(length / 2, 25);
	const tangent = turf.bearing(
		turf.along(line, Math.max(0, at - span), { units: "kilometers" }),
		turf.along(line, Math.min(length, at + span), { units: "kilometers" }),
	);

	const toB = turf.bearing(turf.centerOfMass(featA), turf.centerOfMass(featB));
	return Math.abs(angleDelta(tangent + 90, toB)) <= 90
		? tangent + 90
		: tangent - 90;
}
...
function seaCableAnchor(featA, featB) {
	...
	for (const lineA of outlines(featA)) {
		for (const coord of densify(lineA, 40)) {
			...
			for (const lineB of outlines(featB)) {
				const snapped = turf.nearestPointOnLine(lineB, pt, {
					units: "kilometers",
				});
	...
	if (!best) return null;
	return {
		anchor: turf.midpoint(turf.point(best.a), turf.point(best.b)).geometry
			.coordinates,
...
/** Shared-boundary line per land border, so decollide can slide along it. */
const borderLines = new Map();
let land = 0;
let sea = 0;
...
for (const border of BORDERS) {
	...
	if (!featA || !featB) {
		console.warn(
			`skipping border ${border.isoA}-${border.isoB}: missing polygon`,
		);
		continue;
	}
	...
	if (run.length >= MIN_SHARED_VERTICES) {
		({ anchor, bearing, line } = landAnchor(run, featA, featB));
		kind = "land";
		land++;
	} else {
	...
	if (line) borderLines.set(`${border.a}|${border.b}`, { line, featA, featB });
 ...
 *
 * Land anchors are snapped back onto their own shared boundary after every
 * nudge, so separating them slides the arrow along the border rather than
 * lifting it off into one of the countries.
 */
function decollide(features, minSeparationKm = 85, passes = 40) {
	for (let pass = 0; pass < passes; pass++) {
		...
		for (let i = 0; i < features.length; i++) {
			for (let j = i + 1; j < features.length; j++) {
				...
				const away =
					d > 0.001 ? turf.bearing(p, q) : features[i].properties.bearing + 90;
				const shift = (minSeparationKm - d) / 2 + 1;
				features[i].geometry.coordinates = onBorder(
					features[i],
					turf.destination(p, shift, away + 180, { units: "kilometers" })
						.geometry.coordinates,
				);
				features[j].geometry.coordinates = onBorder(
					features[j],
					turf.destination(q, shift, away, { units: "kilometers" }).geometry
						.coordinates,
				);
				moved = true;
			}
	...
	for (const f of features) {
		const border = borderLines.get(f.id);
		if (border) {
			f.properties.bearing = round(
				crossingBearing(
					border.line,
...
/** Constrain a nudged land anchor to its shared boundary; sea anchors move freely. */
function onBorder(feature, coords) {
	const border = borderLines.get(feature.id);
	if (!border) return coords;
	return turf.nearestPointOnLine(border.line, turf.point(coords), {
		units: "kilometers",
	}).geometry.coordinates;
}
