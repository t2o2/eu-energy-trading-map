...
function selectSource(): GridSource {
  ...
  if (token && process.env.DATA_SOURCE !== 'mock') return new EntsoeSource(token);
...
export async function getSnapshot(): Promise<GridSnapshot> {
  ...
  inFlight = source
    ...
    .catch((err) => {
      if (cache && Date.now() - Date.parse(cache.snapshot.fetchedAt) < STALE_MS) {
...
export function activeSourceName(): 'mock' | 'entsoe' {
  return selectSource().name;
}
