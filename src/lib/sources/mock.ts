...
const FLEETS: Record<string, Fleet> = {
  ...
  AT: { load: 8200, mix: { hydro: 7000, wind: 2500, gas: 2200, solar: 2400, biomass: 500 } },
  BA: { load: 1600, mix: { coal: 1400, hydro: 900, wind: 130 } },
  BE: { load: 10500, mix: { nuclear: 4000, gas: 5200, wind: 3200, solar: 3500, biomass: 700 } },
  BG: { load: 4500, mix: { nuclear: 2000, lignite: 2200, hydro: 900, wind: 500, solar: 1600 } },
  CH: { load: 7200, mix: { hydro: 8000, nuclear: 2200, solar: 2000, pumpedStorage: 1500 } },
  CZ: { load: 8000, mix: { nuclear: 4000, lignite: 4200, gas: 900, solar: 1800, biomass: 500 } },
  DE: { load: 62000, mix: { wind: 42000, solar: 30000, lignite: 12000, coal: 8000, gas: 14000, biomass: 5500, hydro: 3500 } },
  DK: { load: 4200, mix: { wind: 6500, solar: 2500, biomass: 1500, coal: 600 } },
  EE: { load: 950, mix: { oil: 700, wind: 400, biomass: 200 } },
  ES: { load: 30000, mix: { wind: 22000, solar: 25000, nuclear: 6000, gas: 12000, hydro: 8000 } },
  FI: { load: 11000, mix: { nuclear: 4400, hydro: 3200, wind: 6000, biomass: 2000 } },
  FR: { load: 58000, mix: { nuclear: 48000, hydro: 12000, wind: 18000, solar: 12000, gas: 6000 } },
  GB: { load: 34000, mix: { wind: 24000, gas: 20000, nuclear: 5000, solar: 9000, biomass: 3000 } },
  GR: { load: 6500, mix: { gas: 4500, wind: 3500, solar: 5000, lignite: 800, hydro: 1500 } },
  HR: { load: 2200, mix: { hydro: 1800, wind: 900, gas: 700 } },
  HU: { load: 5500, mix: { nuclear: 1900, gas: 1800, solar: 4000, lignite: 500 } },
  IE: { load: 4800, mix: { wind: 4500, gas: 3500, solar: 700 } },
  IT: { load: 38000, mix: { gas: 25000, solar: 20000, hydro: 12000, wind: 10000, geothermal: 800, coal: 2000 } },
  ...
  NL: { load: 13500, mix: { gas: 12000, wind: 9000, solar: 12000, coal: 3000, biomass: 800 } },
  NO: { load: 18000, mix: { hydro: 31000, wind: 5000 } },
  PL: { load: 22000, mix: { coal: 14000, lignite: 7000, wind: 8000, solar: 9000, gas: 3000 } },
  PT: { load: 6000, mix: { wind: 5500, hydro: 5000, solar: 3500, gas: 3000 } },
  RO: { load: 7500, mix: { nuclear: 1300, hydro: 4000, coal: 1800, gas: 2000, wind: 3000, solar: 1500 } },
  RS: { load: 5500, mix: { lignite: 3800, hydro: 2000, wind: 500 } },
  SE: { load: 18000, mix: { hydro: 14000, nuclear: 6500, wind: 12000, biomass: 1500 } },
  SI: { load: 1600, mix: { nuclear: 700, hydro: 1000, lignite: 500, solar: 700 } },
  SK: { load: 3400, mix: { nuclear: 2400, hydro: 900, gas: 500, solar: 500 } },
  TR: { load: 42000, mix: { gas: 22000, coal: 18000, hydro: 20000, wind: 11000, solar: 10000, geothermal: 1600 } },
  UA: { load: 14000, mix: { nuclear: 7000, coal: 5000, hydro: 3000, solar: 5000, wind: 1500 } },
...
function solarFactor(hourUtc: number, lonOffsetHours: number, month: number): number {
...
const LON_HOURS: Record<string, number> = {
  PT: -0.6, IE: -0.5, GB: 0, ES: -0.2, FR: 0.15, BE: 0.3, NL: 0.35, LU: 0.4,
  CH: 0.5, IT: 0.8, DE: 0.7, DK: 0.7, NO: 0.7, SE: 1.0, AT: 0.9, SI: 1.0,
  HR: 1.05, CZ: 1.0, PL: 1.3, HU: 1.3, SK: 1.3, BA: 1.2, ME: 1.3, RS: 1.4,
  AL: 1.3, XK: 1.4, MK: 1.5, GR: 1.55, BG: 1.7, RO: 1.7, MD: 1.9, UA: 2.0,
  FI: 1.7, EE: 1.7, LV: 1.6, LT: 1.6, TR: 2.3,
};
...
export class MockSource implements GridSource {
  ...
  async fetchSnapshot(): Promise<GridSnapshot> {
    ...
    for (const area of AREAS) {
      ...
      const load = Math.round(fleet.load * loadFactor(hourUtc, lon) * (0.95 + 0.1 * hash(seed, area.iso2 + 'L')));

      // Wind is regionally correlated: neighbours share a weather front.
      const windRegion = hash(seed, `wind:${Math.round((LON_HOURS[area.iso2] ?? 1) * 2)}`);
      const windLocal = hash(seed, area.iso2 + 'W');
      const wind = 0.15 + 0.7 * (0.65 * windRegion + 0.35 * windLocal);
      const sun = solarFactor(hourUtc, lon, month) * (0.6 + 0.4 * hash(seed, area.iso2 + 'S'));

      const generation: Partial<Record<ProductionType, number>> = {};
      for (const [t, capacity] of Object.entries(fleet.mix) as [ProductionType, number][]) {
        let factor: number;
        switch (t) {
          case 'wind': factor = wind; break;
          case 'solar': factor = sun; break;
          case 'nuclear': factor = 0.82 + 0.12 * hash(seed, area.iso2 + t); break;
          case 'hydro': factor = 0.35 + 0.3 * hash(seed, area.iso2 + t); break;
          case 'biomass': case 'geothermal': case 'waste': factor = 0.7; break;
          case 'pumpedStorage': factor = Math.max(0, 0.5 - sun) * hash(seed, area.iso2 + t); break;
          default: factor = 0.25 + 0.45 * hash(seed, area.iso2 + t); break; // thermal follows residual demand
        }
      ...
      const total = Object.values(generation).reduce<number>((a, b) => a + (b ?? 0), 0);
      const target = load * (0.9 + 0.2 * hash(seed, area.iso2 + 'T'));
      const thermal: ProductionType[] = ['gas', 'coal', 'lignite', 'oil'];
      const thermalTotal = thermal.reduce((a, t) => a + (generation[t] ?? 0), 0);
      if (thermalTotal > 0) {
        const adjust = Math.max(0.05, Math.min(2.2, 1 + (target - total) / thermalTotal));
        for (const t of thermal) {
          if (generation[t]) generation[t] = Math.round(generation[t]! * adjust);
        }
      }

      const gen = Object.values(generation).reduce<number>((a, b) => a + (b ?? 0), 0);
      const price = Math.round(
        (12 + 180 * Math.max(0, 1 - gen / Math.max(load, 1)) + 45 * hash(seed, area.iso2 + 'P')) * 10,
      ) / 10;
    ...
    for (const border of BORDERS) {
      ...
      const signed = Math.max(-capacity, Math.min(capacity, gradient * 0.28 + noise));
      const mw = Math.round(Math.abs(signed));
      if (mw < 15) continue;

      // A real border almost always has some counter-flow from bilateral trades.
      const counter = Math.round(mw * 0.12 * hash(seed, border.isoA + border.isoB + 'R'));
      flows.push(
        signed >= 0
          ? { from: border.a, to: border.b, netMw: mw, forwardMw: mw + counter, reverseMw: counter }
          : { from: border.b, to: border.a, netMw: mw, forwardMw: mw + counter, reverseMw: counter },
      );
    }
