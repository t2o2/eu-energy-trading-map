...
export class EntsoeSource implements GridSource {
	...
	async fetchSnapshot(): Promise<GridSnapshot> {
			...
			limit(async () => {
				const units = await scalar(
					async () => {
						...
						// One unit can appear as several series (generation + consumption
						// legs, or split periods), so accumulate per EIC.
						...
						for (const ts of series) {
							...
							// A pumping unit is published as a consumption series; carry it
							// as negative so a plant's net output stays honest.
		...
		const [areas, flows, unitGroups] = await Promise.all([
