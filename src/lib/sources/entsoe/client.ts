...
function parseTimeSeries(doc: XmlNode): TimeSeries[] {
  ...
  for (const ts of toArray(root.TimeSeries as XmlNode | XmlNode[])) {
    ...
    const isConsumption = 'outBiddingZone_Domain.mRID' in ts && !('inBiddingZone_Domain.mRID' in ts);
    ...
    for (const period of toArray(ts.Period as XmlNode | XmlNode[])) {
      ...
      if (points.length > 0) {
        series.push({ psrType, start, resolution, points, isConsumption });
      }
    }
  }
  return series;
...
export function latestValue(ts: TimeSeries, now: Date): number | null {
  ...
  for (const p of ts.points) {
    if (p.position <= maxPosition && (best === null || p.position > best.position)) {
      best = p;
    }
  }
...
export async function query(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    let res: Response;
    try {
      res = await fetch(url, { signal, headers: { Accept: 'application/xml' } });
    } catch (err) {
